...
def save_chat(chat_data: dict[str, Any]) -> None:
    ...
    file_path = SAVED_CHATS_PATH / f"{chat_data['id']}.json"
...
def load_chat(chat_id: str) -> dict[str, Any] | None:
    file_path = SAVED_CHATS_PATH / f"{chat_id}.json"
...
def load_all_chats() -> list[dict[str, Any]]:
    ensure_chat_folder()
    chats = []
    for file_path in SAVED_CHATS_PATH.glob("*.json"):
        try:
            with file_path.open("r", encoding="utf-8") as file:
                chat_data = json.load(file)
            if isinstance(chat_data, dict) and chat_data.get("id"):
...
def rename_chat(chat_id: str, new_title: str) -> bool:
    cleaned_title = " ".join(new_title.strip().split())
...
def generate_chat_title(message: str, max_length: int = 38) -> str:
    cleaned_message = " ".join(message.strip().split())
    if not cleaned_message:
        return "Yeni Sohbet"
    if len(cleaned_message) <= max_length:
        return cleaned_message
    return cleaned_message[:max_length].rstrip() + "..."

def chat_to_text(chat_data: dict[str, Any]) -> str:
    lines = [f"Sohbet Başlığı: {chat_data.get('title', 'Yeni Sohbet')}", ""]
    for message in chat_data.get("messages", []):
        ...
        if role == "user":
            ...
            speaker = "LocalStudyAI"
        else:
            continue
        lines.extend([f"{speaker}:", content, "", "-" * 50, ""])
    return "\n".join(lines)

def chat_to_json(chat_data: dict[str, Any]) -> str:
